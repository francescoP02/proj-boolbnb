    ...
    <form action="{{ route('admin.apartments.update', ['apartment' => $apartment->id]) }}" method="POST"
        ...
        <div class="mb-3">
            ...
            <div class="d-none" id="containerAddress">
                <select name="address" id="address" class="form-select" onclick="controlForm()">
                    ...
                    <option value="{{ $apartment->address }}"
                        {{ old('address', $apartment->address) == $i ? 'selected' : '' }}></option>
                </select>
            </div>
        </div>
        ...
        <div class="my-3">
            ...
            @foreach ($optionals as $optional)
                <div class="form-check">
                    ...
                    <input name="optionals[]" class="form-check-input opt" onclick="controlForm()" type="checkbox"
                        value="{{ $optional->id }}" id="optional-{{ $optional->id }}"
                        {{ $apartment->optionals->contains($optional) || in_array($optional->id, old('optionals', [])) ? 'checked' : '' }}>
        ...
        <div class="d-none">
            ...
            <input id="lonApart" name="longitude" readonly
                value="{{ old('longitude') ? old('longitude') : $apartment->longitude }}">
        </div>
